...
class MarketDataValidator:
    ...
    def normalize_ticker(cls, ticker: str) -> str:
        """
        Canonical ticker normalizer. Delegates to authoritative universe_config.normalize_ticker.
        """
    ...
    def validate_research_tickers(
        ...
        """
        Authoritative validation pipeline for research ticker symbols:
        Uses central validate_ticker_list backed by the 500+ Indian equity universe.
        Fails closed immediately on unknown symbols (like DSDSDS.NS) without network lag.
        Logs rejection events to MasterLogger.
        """
        from app.analytics.master_logger import MasterLogger
        from app.analytics.universe_config import validate_ticker_list

        ok, clean_tickers, err_msg = validate_ticker_list(raw_symbols)
        if not ok:
            # Extract ticker for logger if single symbol
            tkr = str(raw_symbols).strip().upper() if isinstance(raw_symbols, str) and "," not in raw_symbols else None
            MasterLogger.log_event("DATA_GATE", "REJECTED_INVALID_TICKER", err_msg or "Validation failed", ticker=tkr, severity="WARNING")
